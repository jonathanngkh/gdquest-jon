...
func part_010_adding_bridges() -> void:
	...
	bubble_add_text([
		...
		gtr("Alternatively, you can press press [b]F6[/b] on your keyboard."),
		gtr("Then, press [b]F8[/b] or close the game window to stop the game."),
		]
	)
	...
	bubble_add_text([
		...
		gtr("Click the [b]Play Edited Scene[/b] button in the top-right or press [b]F6[/b] to run the scene."),
		gtr("Press [b]F8[/b] to close the game window once you're done."),
	])
	bubble_add_task_press_button(interface.run_bar_play_current_button, "Play Edited Scene")
	complete_step()
